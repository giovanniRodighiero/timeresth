...
import { render } from "../../../tools/testUtils";
...
describe("<Title />", () => {
    ...
    it.each([
        ...
        const { getByText } = render(
            <Title color={color}>title of the page</Title>
        );
    ...
    it.each([
        ...
        "Should create tag heading of level %s for prop %s.",
        (expected, tag) => {
            const { getByRole } = render(
                <Title tag={tag}>title of the page</Title>
            );

            expect(
                getByRole("heading", { level: expected })
            ).toBeInTheDocument();
        }
    );
});
